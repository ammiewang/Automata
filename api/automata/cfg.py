...
class CFG:
    ...
    def find_terms_nonterms(self):
        self.terminals = set()
        self.nonterminals = set()#set(self.rules.keys())#set()
        for nt, prods in self.rules.items():
            for str in prods:
                for char in str: #x str.split()
                    term = self.is_terminal(char)
                    if term: #and char not in self.terminals:
                        self.terminals.add(char)
                    else: #elif not term and char not in self.nonterminals:
        ...
        while cont:
            ...
            for nt in curr:
                if nt in self.rules:
                    ...
                    for str in prods:
                        for char in str: #x str.split()
                            if not self.is_terminal(char) and char not in visited: #x and char != ' '
                                next.add(char)
                                visited.add(char)
                                cont = True
            curr = next
    ...
    def elim_nonterminating(self):
        ...
        while cont:
            ...
            for nt, prods in curr.items():
                ...
                for str in prods:
                    include = True
                    for char in str: #x str.split()  and char != ' '
        ...
        for nt, prod in self.rules.items():
            for str in prod:
                if len(str) == 1 and not self.is_terminal(str): #change isupper in case of list
                    if nt in unit_prods:
                        #unit_prods[nt].append(str[0])
                        unit_prods[nt].add(str[0])
                    else:
                        #unit_prods[nt] = [str[0]]
                        unit_prods[nt] = set([str[0]])
                else:
    ...
    def find_unit_prod_pairs(self):
        unit_productions, not_unit_prods = self.unit_prods() #both dictionaries {nt: [['A', 'B'], ['c']]}

        new_unit_prod = {}
        for nt,prods in unit_productions.items():
            new_prods = set()
            #new_prods = []
            curr_prods = prods.copy() #[[A,B], [c]]
            while len(curr_prods) > 0:
                ...
                for nt2 in curr_prods:
                    if nt2 not in new_prods:
                        new_prods.add(nt2)
                        #new_prods.append(nt2)
                        # if len(nt2) == 1 and nt2[0] in unit_productions:
                        #     for str in unit_productions[nt2[0]]:
                        if nt2 in unit_productions:
                            # for str in unit_productions[nt2]:
                            #     if str not in add_to_prods:
                            #         add_to_prods.append(str)
    ...
    def elim_unit(self):
        # use graph alg instead
        # unit_prods = {}
        # unit_ancestors = {nt: set() for nt in self.rules}
        # Q = deque([self.start])
        #
        # while Q:
        #     nt = Q.popleft()
        #     for prod in self.rules[nt]:
        #         if len(prod) == 1 and not self.is_terminal(prod[0]):
        #             char = prod[0]
        #             if nt in unit_prods:
        #                 unit_prods[nt].add(char)
        #             else:
        #                 unit_prods[nt] = {char}
        #             for nt2 in unit_ancestors[nt]:
        #                 unit_prods[nt2].add(char)
        #             if char in unit_prods:
        #                 for nt3 in unit_prods[char]:
        #                     for nt4 in unit_ancestors[nt].union(nt):
        #                         unit_prods[nt4].add(nt3)
        #             unit_ancestors[char].add(nt)
        #             unit_ancestors[char] = unit_ancestors[char].union(unit_ancestors[nt])
        #             if char not in unit_prods and char not in Q:
        #                 Q.append(char)
        #         else:
        #             for char in prod:
        #                 if not self.is_terminal(char) and char not in unit_prods and char not in Q:
        #                     Q.append(char)

        new_no_unit_prod = {}
        unit_prods, not_unit_prods = self.find_unit_prod_pairs()
        #unit_prods = {nt: set}, not_unit_prods = {nt: 2d arr}
    ...
    def enum_strs(self, str, all_letters, null_prods):
        if len(all_letters) == 0:
            ...
            new_str = str + [all_letters[0]]
            if all_letters[0] not in null_prods:
                return self.enum_strs(new_str, all_letters[1:], null_prods) #x str + ' ' + all_letters[0]
            else:
                #x str + ' ' + all_letters[0]
                return self.enum_strs(new_str, all_letters[1:], null_prods) + \
    ...
    def elim_null(self):
        ...
        nts_to_replace = self.find_null_prod(new_grammar) #set, if B -> Bd, k, epsilon, would be {B}
        ...
        while cont:
            ...
            for nt, prods in strs.items():
                for str in prods:
                    new_prods = self.enum_strs([], str, nts_to_replace)
    ...
    def new_ss(self):
        ...
        self.rules[new_start] = [[self.start]] #x list change
    ...
    def replace_terms(self):
        ...
        already_exists = set()
        for nt, prods in self.rules.items():
            if len(prods) == 1 and len(prods[0]) == 1 and self.is_terminal(prods[0][0]):
                already_exists.add(prods[0][0])
                new_nt_rev[prods[0][0]] = nt
    ...
    def check(self, prods, i):
        ...
        return new_nt, rep, same

    def subs(self, i):
        new_prod = []
        nt = self.ordering[i]
        new_nt, rep, lt = self.check(self.rules[nt], i)
        to_rm = new_nt + rep#new_nt.union(rep)
        all_left_recs = new_nt[:]#set(new_nt.copy())
        ...
        while cont:
            ...
            for prod in rep:
                subbed = self.substitute(prod)
                eq, gt, lt = self.check(subbed, i)
                nt2 += gt
                all_left_recs += eq
    ...
    def elim_left_rec(self, strs, new_nt):
        ...
        for str in strs:
            postfix = str[1:]
            dict[new_nt] += [postfix + [new_nt], postfix]#[postfix + new_nt, postfix] #does the new sym that should be AB | ep
        return dict
    ...
    def prod_first_set(self, prod):
        ...
        if self.is_terminal(prod[0]):
            ...
            while len(prod) > 0 and not self.is_terminal(prod[0]) and 'ε' in self.firsts[prod[0]]:
                first = first.union(self.firsts[prod[0]]).difference('ε') #fs[prod[0]]
                prod = prod[1:]
            if len(prod) == 0:
                first.add('ε')
            ...
            for prod in prods:
                if self.is_terminal(prod[0]):
                    ...
                    while str[0] in eps:
                        str = str[1:]
                        if len(str) > 0:
                            also_inherits_from[nt].add(str[0])
    ...
    def predict_sets(self):
        ...
        for nt, prods in self.rules.items():
            for prod in prods:
                ...
                if 'ε' not in pfs:
                    predict[(nt,' '.join(prod))] = pfs
                    #predict[(nt,prod)] = pfs
                else:
                    predict[(nt,' '.join(prod))] = pfs.difference('ε').union(self.follows[nt])
